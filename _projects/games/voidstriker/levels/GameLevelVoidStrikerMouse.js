...
class GameLevelVoidStrikerMouse {
  constructor(gameEnv) {
    ...
    setTimeout(() => VoidStrikerMouseGame.init(gameEnv), 200);
...
const VoidStrikerMouseGame = (() => {
  ...
  // Mouse cursor position relative to the canvas — null until the first mousemove
  ...
  function updateShip() {
    ...
    // Mouse-aimed continuous shooting — fires every cooldown toward cursor
    if (ship.shootCooldown === 0 && mouseX !== null) {
      const sx = mouseX - ship.x;
      const sy = mouseY - ship.y;
  ...
  function buildTitleScreen() {
    ...
    div.innerHTML = `
      ...
      <div style="font-size:14px; opacity:0.7; margin-bottom:28px; letter-spacing:2px;">WASD to move &nbsp;•&nbsp; Mouse to aim &amp; shoot &nbsp;•&nbsp; P to pause / select ship</div>
  ...
  function attachInput() {
    window.addEventListener('keydown', e => {
      ...
      // Stop space from scrolling the host page while the game has focus
      if (e.key === ' ') e.preventDefault();
    });
    window.addEventListener('keyup', e => { keys[e.key] = false; });

    // Track mouse position relative to the canvas for aiming
...
export default GameLevelVoidStrikerMouse;
